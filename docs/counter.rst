...
.. Important:: If you wish to download the top file instead, go :download:`here </files/DUT_counter/myip_counter_v1_0_S00_AXI.v>`. 
...
When using the AXI VIP, there are two packages that you must import that are shown in the code block below.
...
add a clk and reset bit and initialize them both to zero. After this, I create names for addresses and data I will send, but this is optional,
you can insert the addresses and data directly into the commands. Next, instanciate the block design from the wrapper file. From there, it
is necessary to create a master agent vip, create an agent, and start the agent (using appropriate hiarchy as well).
